...
function setupListeners(dispatch, getState, adapterToUse) {
    logger.info(
        `DEBUG: adapterActions->setupListeners 
        adapterToUse ${JSON.stringify(adapterToUse)}`
    );
...
function onDeviceConnected(dispatch, getState, device) {
    ...
    logger.debug(`TARENT onDeviceConnected
        bondInfo: ${JSON.stringify(bondInfo)}`);
...
function onConnParamUpdateRequest(
    ...
    logger.debug(`TARENT onConnParamUpdateRequest
        reqConnParams: ${JSON.stringify(requestedConnectionParams)}`);
...
function onConnParamUpdate(dispatch, getState, device, connectionParams) {
    logger.debug(`TARENT onConnParamUpdate
        connParams: ${JSON.stringify(connectionParams)}`);
...
function onAttMtuChanged(dispatch, getState, device, mtu) {
    logger.debug(`TARENT onAttMtuChanged
        mtu: ${JSON.stringify(mtu)}`);
...
function onAttMtuRequest(dispatch, getState, device, mtu) {
    logger.debug(`TARENT onAttMtuRequest
        mtu: ${JSON.stringify(mtu)}`);
    if (getState().app.adapter.autoConnUpdate === true) {
...
function onPhyUpdateRequest(dispatch, getState, device, requestedPhyParams) {
    logger.debug(`TARENT onPhyUpdateRequest
        requestedPhyParams: ${JSON.stringify(requestedPhyParams)}`);
    if (getState().app.adapter.autoConnUpdate === true) {
        const phyParams = {
...
function onPhyUpdated(dispatch, getState, device) {
    logger.debug(`TARENT onPhyUpdated
...
function onDataLengthUpdateRequest(
    ...
    logger.debug(`TARENT onDataLengthUpdateRequest
        dataLength: ${JSON.stringify(requestedDataLengthParams)}`);
    if (getState().app.adapter.autoConnUpdate === true) {
...
function onDataLengthUpdated(dispatch, getState, device) {
    logger.debug(`TARENT onDataLengthUpdated
        device: ${JSON.stringify(device)}`);
...
function onSecurityRequest(dispatch, getState, device, params) {
    ...
    logger.debug(`TARENT onSecurityRequest
        defaultSecParams: ${JSON.stringify(defaultSecParams)}`);
...
function onSecParamsRequest(dispatch, getState, device, peerParams) {
    ...
    logger.debug(`TARENT onSecParamsRequest, computePublicKey
                keyset: ${JSON.stringify(secKeyset)} 
                pk: ${JSON.stringify(secKeyset.keys_own.pk)}`);
    logger.debug(`TARENT onSecParamsRequest, device: ${JSON.stringify(device)}
                check device._role which has diff`);

    // TARENT NOTICE: focus to role=p, not role=c which we control the dongle as host
    if (device.role === 'central') {
        logger.debug(`TARENT onSecParamsRequest, role=c, ownPeriphInitiatedPairingPending: 
        ...
        if (device.ownPeriphInitiatedPairingPending) {
            ...
            logger.debug(`TARENT onSecParamsRequest, role=c, replySecParams
                periphInitSecParams: ${JSON.stringify(periphInitSecParams)}
            ...
            logger.debug(`TARENT onSecParamsRequest, role=c, autoAcceptPairing:
                ${selectedAdapter.security.autoAcceptPairing}`);
            ...
            logger.debug(`TARENT onSecParamsRequest, role=c, securityRequestAction
                secParams: ${secParams}`);
            dispatch(securityRequestAction(device, secParams));
        }
    } else if (device.role === 'peripheral') {
        logger.debug(`TARENT onSecParamsRequest, role=p, replySecParams
            secKeyset: ${JSON.stringify(secKeyset)}`);
        adapterToUse.replySecParams(
            device.instanceId,
            0,
            ...
            error => {
                ...
                logger.debug(`TARENT onSecParamsRequest, role=p, replySecParams response
                    error: ${error}
                    secKeyset: ${JSON.stringify(secKeyset)}`);
            }
        );
...
function onSecInfoRequest(dispatch, getState, device) {
    ...
    logger.debug(`TARENT onSecInfoRequest
        bondInfo: ${JSON.stringify(bondInfo)}`);
...
function onAuthKeyRequest(dispatch, getState, device, keyType) {
    ...
    logger.debug(`TARENT onAuthKeyRequest
        sendKeyPress: ${JSON.stringify(sendKeyPress)}`);
...
function onLescDhkeyRequest(
    ...
    logger.debug(`TARENT onLescDhkeyRequest, computeSharedSecret
                peerPublicKey: ${JSON.stringify(peerPublicKey)}
                dhKey: ${dhKey}`);
    ...
    adapterToUse.replyLescDhkey(device.instanceId, dhKey, error => {
        ...
        logger.debug(`TARENT onLescDhkeyRequest, replyLescDhkey
                    error: ${error}
                    dhKey: ${dhKey}`);
    });

    const publicKey = adapterToUse.computePublicKey();
    logger.debug(`TARENT onLescDhkeyRequest, computePublicKey
                publicKey: ${publicKey}`);
    adapterToUse.getLescOobData(
        ...
        (error, ownOobData) => {
            ...
            logger.debug(`TARENT onLescDhkeyRequest, getLescOobData
                        error: ${error}
                        ownOobData: ${JSON.stringify(ownOobData)}`);
...
function authenticate(dispatch, getState, device, securityParams) {
    ...
    return new Promise((resolve, reject) => {
        logger.debug(`TARENT authenticate
            securityParams: ${JSON.stringify(securityParams)}`);
        ...
        new Promise((resolve, reject) => {
            ...
            adapterToUse.authenticate(
                ...
                error => {
                    ...
                    logger.debug(`TARENT Authenticate, kdist_own: 
                        ${JSON.stringify(securityParams.kdist_own)}`);
                    logger.debug(`TARENT Authenticate, kdist_peer:
                        ${JSON.stringify(securityParams.kdist_peer)}`);
                    resolve();
                }
            );
        })
            .then(() => {
                ...
                logger.debug(`TARENT pairWithDevice, store param status action,
                     id: ${id} dev: ${device} 
                     secParams: ${securityParams}
                     own: ${JSON.stringify(securityParams.kdist_own)} 
                     peer: ${JSON.stringify(securityParams.kdist_peer)}`);
            })
            .catch(error => {
...
export function acceptPairing(id, device, securityParams) {
    return (dispatch, getState) =>
        new Promise((resolve, reject) => {
            ...
            secKeyset.keys_own.pk = { pk: adapterToUse.computePublicKey() };
            logger.debug(`TARENT acceptPairing, computePublicKey
                keyset: ${secKeyset} pk: ${secKeyset.keys_own.pk}`);
            logger.debug(`TARENT acceptParaing, role: ${device.role}
                p will authenticate, c will replySecParams`);
