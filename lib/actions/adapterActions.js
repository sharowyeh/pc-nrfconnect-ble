...
function onSecParamsRequest(dispatch, getState, device, peerParams) {
    ...
    if (device.role === 'central') {
        ...
        if (device.ownPeriphInitiatedPairingPending) {
            ...
            logger.debug(`TARENT onSecParamsRequest, replySecParams role c
            ...
            logger.debug(`TARENT onSecParamsRequest, autoAcceptPairing: 
            ...
            logger.debug(`TARENT onSecParamsRequest, securityRequestAction
                secParams: ${secParams}`);
            dispatch(securityRequestAction(device, secParams));
        }
    } else if (device.role === 'peripheral') {
        logger.debug(`TARENT onSecParamsRequest, replySecParams role p
            secKeyset: ${JSON.stringify(secKeyset)}`);
        adapterToUse.replySecParams(
            device.instanceId,
            0,
        ...
        new Promise((resolve, reject) => {
            ...
            .then(() => {
                ...
                logger.debug(`TARENT pairWithDevice, store param status action,
                     id: ${id} dev: ${device} secParams: ${securityParams}
                     kdist own: ${JSON.stringify(securityParams.kdist_own)} 
