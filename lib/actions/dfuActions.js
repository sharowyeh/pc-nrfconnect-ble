...
function getDeviceByAddress(adapter, address) {
    ...
    const foundDeviceId = Object.keys(devices).find(
        deviceId => devices[deviceId].address === address
    );
...
function onLogMessage(severity, message) {
    switch (severity) {
        case TRACE:
        case DEBUG:
            logger.debug(`onLogMessage ${message}`);
            break;
        case INFO:
            logger.info(message);
            break;
...
export function showDfuDialog(device) {
    return (dispatch, getState) => {
        ...
        const getBondingData = () => {
            const bondInfo = state.app.adapter.getIn([
                'selectedAdapter',
                'security',
                'bondStore',
                device.address,
    ...
    return (dispatch, getState) => {
        ...
        const intToAddress = integer => {
            ...
            return bytestring.replace(
                MATCH_CHARACTER_PAIRS_NOT_AT_END_OF_LINE,
                ADD_COLON
            );
        };
